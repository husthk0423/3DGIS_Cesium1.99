...
function CesiumTerrainProvider(options) {
  ...
  // let _levelZero = this.tileSize == 512?1:0;
  this.requestZeroLevel  = defaultValue(options.requestZeroLevel,false);
  this.hasminMaxHeightConfig = true;

  Promise.resolve(options.url)
    .then(function (url) {
      let promise = [];
      if(that.hasminMaxHeightConfig){
        const resourceH = Resource.createIfNeeded(url);
        resourceH.appendForwardSlash();
        var JsonResource = resourceH.getDerivedResource({
          url: "minMaxHeight.json",
        });

        promise = JsonResource.fetchJson();
      }

      Promise.resolve(promise).then(function(data){
        that.maxminTerrainHeight = data;
      }).finally(function(){
  ...
  function parseMetadataSuccess(data) {
    ...
    const availabilityLevels = undefined;
    that.availabilityLevels = availabilityLevels;
...
function requestTileGeometry(provider, x, y, level, layerToUse, request) {
  ...
  replaceSubdomain(resource,provider._subdomains);
  if (
  ...
  if(!provider.requestZeroLevel && level == 0 && !defined(provider.availabilityLevels)){
    const deferred = defer();
...
function replaceSubdomain(resource,subdomains){
  if(subdomains.length < 1){
    return;
  }
  var reg = /(?<=^http(s)?:\/\/)(.*?)\//;
  var index = Math.round(Math.random()*(subdomains.length - 1));
  var ToReplace = subdomains[index]+'/';
  resource._url = resource._url.replace(reg, ToReplace);
}
...
CesiumTerrainProvider.prototype.getTileDataAvailable = function (x, y, level) {
  ...
  if (this._availability.isTileAvailable(level, x, y)) {
    ...
    if(this.maxminTerrainHeight){
      let terrainY = getTerrainY(this,level,y);
      let key = x + '_'+terrainY+'_'+level;
      let maxminTerrainHeight = this.maxminTerrainHeight[key];
      let b = maxminTerrainHeight?true:false;
      // if(!b){
      //   console.log(key);
      // }
      return b;
...
function getTerrainY(terrainProvider,level,y){
  let terrainY = y;
  if (!terrainProvider._scheme || terrainProvider._scheme === "tms") {
    const yTiles = terrainProvider._tilingScheme.getNumberOfYTilesAtLevel(level);
    terrainY = yTiles - y - 1;
  }
  return terrainY;
}
