...
/*************新增开始**************/
import PrimitivePlugin from '../primitivePlugin/PrimitivePlugin.js';
import LightTool from '../util/LightTool';
/*************新增结束**************/
/**
...
function Primitive(options) {
  ...
  /*************新增开始**************/
  //显示mask
  this.undisplayable = options.undisplayable == null ? false : options.undisplayable;
  this.focusable = options.focusable == null ? false : options.focusable;
  this.primitivePlugin = new PrimitivePlugin();

  if(this.undisplayable == true){
    this.primitivePlugin.addPlugin("undisplayable");
  }
  if(this.focusable == true){
    this.primitivePlugin.addPlugin("focusable");
  }
  this.primitivePlugin.build();
...
function createShaderProgram(primitive, frameState, appearance) {
  ...
  /*************新增开始**************/
  fs = primitive._batchTable.setFSAttributesHeader(fs);
  fs = primitive.primitivePlugin.addFSPart(fs);
  /*************新增结束**************/
...
function getUniforms(primitive, appearance, material, frameState) {
  ...
  uniforms = combine(uniforms, primitive.uniformsMap);
  return uniforms;
}
...
Primitive.prototype.update = function (frameState) {
  ...
  if (
    ...
    /*************新增开始**************/
    this.propertiesMap = {};
    let instances = (Array.isArray(this.geometryInstances)) ? this.geometryInstances : [this.geometryInstances];
 ...
 */
Primitive.prototype.updateLightShader = function(light){
 ...
 * @param type  光源效果类型
 */
Primitive.prototype.removeLightShader = function(type){
 ...
 * 更新shader
 */
Primitive.prototype.updateShader = function(){
  if(this.appearance){
 ...
 * @param id
 */
Primitive.prototype.getProperties = function (id) {
  if(!this.propertiesMap){
    let json = this.bufferToStr();
    this.propertiesMapBuffer = null;
    if(json){
      this.propertiesMap = JSON.parse(json);
    }else{
      return {};
    }
  }
...
Primitive.prototype.bufferToStr = function () {
  if(this.propertiesMapBuffer){
    var res = '';
    var chunk = 65536;
    var i;
    var array = new Uint16Array(this.propertiesMapBuffer);
