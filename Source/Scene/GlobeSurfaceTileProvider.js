...
function GlobeSurfaceTileProvider(options) {
  ...
  this.baseColor = new Color(0.8, 0.8, 0.8, 1.0);
  ...
  this._excavateAnalysis =  {
    splitNum: 30,//分割点数
    showTailorOnly: false,//是否开启仅展示自己
    tailorArea: undefined,
    enableTailor: false, // 是否开启剪裁
    inverTailorCenterMat: false, // 剪裁相关矩阵
    tailorRect: new Cartesian4(), // 剪裁区域
...
GlobeSurfaceTileProvider.prototype.endUpdate = function (frameState) {
  ...
  for (
    ...
    for (
      ...
      if(this._excavateAnalysis.openUplift){
...
function updateTileBoundingRegion(tile, tileProvider, frameState) {
  ...
  let tileMaxminTerrainHeight =null;
  if(tileProvider._terrainProvider.maxminTerrainHeight){
    let terrainY = getTerrainY(tileProvider,tile.level,tile.y);
    let key = tile.x + '_'+terrainY+'_'+tile.level;
    tileMaxminTerrainHeight = tileProvider._terrainProvider.maxminTerrainHeight[key];
  }

  // if(!tileMaxminTerrainHeight){
  //   tileMaxminTerrainHeight = [0,0];
  // }
  ...
  if (surfaceTile.tileBoundingRegion === undefined) {
    if(tileMaxminTerrainHeight){
      surfaceTile.tileBoundingRegion = new TileBoundingRegion({
        ...
        ellipsoid: ellipsoid,
        minimumHeight: tileMaxminTerrainHeight[0],
        maximumHeight: tileMaxminTerrainHeight[1],
      });
    }else {
      surfaceTile.tileBoundingRegion = new TileBoundingRegion({
        computeBoundingVolumes: false,
  ...
  if(tileProvider._excavateAnalysis.openUplift){
    maximumHeightChange = tileProvider._excavateAnalysis.upHeight;
  }
  ...
  if(tileMaxminTerrainHeight){
    tileBoundingRegion.minimumHeight = tileMaxminTerrainHeight[0];
    tileBoundingRegion.maximumHeight = tileMaxminTerrainHeight[1];
  }else{
    if (
        mesh !== undefined &&
      ...
      while (ancestorTile !== undefined) {
        ...
        if (ancestorSurfaceTile !== undefined) {
          if(!tileProvider._terrainProvider.requestZeroLevel && ancestorTile.level == 0){
            tileBoundingRegion.minimumHeight = ancestorSurfaceTile.tileBoundingRegion.minimumHeight;
            tileBoundingRegion.maximumHeight = ancestorSurfaceTile.tileBoundingRegion.maximumHeight;
            break;
          }
  ...
  tileBoundingRegion.maximumHeight += maximumHeightChange;
...
function getTerrainY(tileProvider,level,y){
  let terrainY = y;
  if (!tileProvider._terrainProvider._scheme || tileProvider._terrainProvider._scheme === "tms") {
    const yTiles = tileProvider._terrainProvider._tilingScheme.getNumberOfYTilesAtLevel(level);
    terrainY = yTiles - y - 1;
  }
  return terrainY;
}
...
function createTileUniformMap(frameState, globeSurfaceTileProvider) {
  const uniformMap = {
    u_showTailorOnly: function() {
      return globeSurfaceTileProvider._excavateAnalysis.showTailorOnly;
    },
    u_tailorArea: function() {
      if (globeSurfaceTileProvider._excavateAnalysis.tailorArea && globeSurfaceTileProvider._excavateAnalysis.tailorArea._colorTextures && globeSurfaceTileProvider._excavateAnalysis.tailorArea._colorTextures[0]) {
        return globeSurfaceTileProvider._excavateAnalysis.tailorArea._colorTextures[0];
      } else {
        return frameState.context.defaultTexture;
      }
    },
    u_enableTailor: function() {
      return globeSurfaceTileProvider._excavateAnalysis.enableTailor;
    },
    u_inverTailorCenterMat: function() {
      return globeSurfaceTileProvider._excavateAnalysis.inverTailorCenterMat || new Matrix4();
    },
    u_tailorRect: function() {
      return globeSurfaceTileProvider._excavateAnalysis.tailorRect || new Cartesian4();
    },
    ...
    u_floodArea: function() {
      if (globeSurfaceTileProvider._floodAnalysis.floodArea && globeSurfaceTileProvider._floodAnalysis.floodArea._colorTextures && globeSurfaceTileProvider._floodAnalysis.floodArea._colorTextures[0]) {
        return globeSurfaceTileProvider._floodAnalysis.floodArea._colorTextures[0];
      } else {
        return frameState.context.defaultTexture;
      }
    },
    u_enableFlood: function() {
      return globeSurfaceTileProvider._floodAnalysis.enableFlood;
    },
    floodVar: function () {
      var arr = globeSurfaceTileProvider._floodAnalysis.floodVar;
      if (arr) {
    ...
    u_inverFloodCenterMat: function() {
      return globeSurfaceTileProvider._floodAnalysis.inverFloodCenterMat || new Matrix4();
    },
    u_floodRect: function() {
      return globeSurfaceTileProvider._floodAnalysis.floodRect || new Cartesian4();
    },
    ...
    u_needUplift: function () {
      return this.properties.needUplift;
    },
...
function addDrawCommandsForTile(tileProvider, tile, frameState,needUplift) {
  const surfaceTile = tile.data;
  ...
  if(needUplift && frameState.camera._scene._globe.terrainExaggeration == 1.0){
    frameState.camera._scene._globe.terrainExaggeration = 1.0001;
  }
  const exaggeration = frameState.terrainExaggeration;
  const terrainExaggerationRelativeHeight =
      frameState.terrainExaggerationRelativeHeight;

  // const exaggerationRelativeHeight = needUplift
  //     ? terrainExaggerationRelativeHeight + tileProvider._excavateAnalysis.upHeight
  //     : terrainExaggerationRelativeHeight;
  const exaggerationRelativeHeight = needUplift
      ? terrainExaggerationRelativeHeight -
      tileProvider._excavateAnalysis.upHeight / (exaggeration - 1)
      : terrainExaggerationRelativeHeight;
  ...
  do {
    ...
    while (numberOfDayTextures < maxTextures && imageryIndex < imageryLen) {
      ...
      if (imagery.imageryLayer._imageryProvider.getTexture) {
        var _texture = imagery.imageryLayer._imageryProvider.getTexture(imagery);
        if (_texture) {
          texture = _texture;
        }
      }
    ...
    uniformMapProperties.needUplift = needUplift;
    ...
    /*************新增开始**************/
    surfaceShaderSetOptions.isUpdatedSharder = tileProvider.isUpdatedSharder;
    tileProvider.isUpdatedSharder = false;
    surfaceShaderSetOptions.lightMap = lightMap;
    /*************新增结束**************/
 ...
 */
GlobeSurfaceTileProvider.prototype.updateLightShader = function (light) {
  lightMap[light.type] = light;
  this.isUpdatedSharder = true;
};
 ...
 * @param type  光源效果类型
 */
GlobeSurfaceTileProvider.prototype.removeLightShader = function (type) {
