...
GlobeSurfaceTile.processStateMachine = function (
  ...
  if (terrainOnly || !surfaceTile.terrainData) { // 增加是否有地形数据判断，保证先加载地形数据，再加载其它图层数据
    return;
  }
...
function processTerrainStateMachine(
  ...
  if (surfaceTile.terrainState === TerrainState.UNLOADED) {
    if(tile.data.boundingVolumeSourceTile){
      let pTile = tile.data.boundingVolumeSourceTile;
      console.log('parentTIle '+ pTile.x + '_'+ pTile.y + '_'+pTile.level);
    }
    console.log('terrain '+ tile.x + '_'+ tile.y + '_'+tile.level);
    requestTileGeometry(
      surfaceTile,
      terrainProvider,
      tile.x,
