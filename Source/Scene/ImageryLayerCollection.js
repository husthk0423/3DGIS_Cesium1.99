...
ImageryLayerCollection.prototype._updateElevationLayers = function() {
  this.elevationProviders = [];
  ...
  for (i = 0, len = layers.length; i < len; ++i) {
    layer = layers[i];
    if(layer.imageryProvider.isElevation){
      layer.imageryProvider.index = i;
      this.elevationProviders.push(layer.imageryProvider);
    }
  }
}
