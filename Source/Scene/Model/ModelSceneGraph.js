...
/***hk����***/
import ModelMultiClippingPlanesPipelineStage from "./ModelMultiClippingPlanesPipelineStage.js";
import ModelMultiClippingPlanesYaPingStage from "./ModelMultiClippingPlanesYaPingStage.js";
...
ModelSceneGraph.prototype.buildDrawCommands = function (frameState) {
  ...
  const sphereMODEE = Matrix4.multiplyByPoint(
      model.modelMatrix,
      modelPositionMin,
      new Cartesian3()
    );
   let cartographic = Cesium.Cartographic.fromCartesian(sphereMODEE);
   
   const sphereMODEEmax= Matrix4.multiplyByPoint(
    model.modelMatrix,
    modelPositionMax,
    new Cartesian3()
  );
  ...
  // let minheight=Cesium.Cartographic.fromCartesian(sphereMODEE).height;


  // for(var m=0;m<testCenterPoints.length;m++)
  // {
  //   let cartographictest= Cesium.Cartographic.fromCartesian(testCenterPoints[m]);
  //   console.log(cartographictest.height);
  // }
  ...
  console.log('######################################################'+modelPositionMin.y);
      const drawCommand = buildDrawCommand(
        ...
        //modelPositionMin.z
        // cartographic.height
        // cartographicmax.height- cartographic.height
      );
...
ModelSceneGraph.prototype.configurePipeline = function (frameState) {
  ...
  /***hk����***/
  if (model.isMultiClippingEnabled()) {
    modelPipelineStages.push(ModelMultiClippingPlanesPipelineStage);
    // modelPipelineStages.push(ModelMultiClippingPlanesYaPingStage);
  }
