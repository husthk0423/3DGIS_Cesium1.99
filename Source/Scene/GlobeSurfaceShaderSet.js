...
/*************新增开始**************/
import LightTool from '../util/LightTool.js';
/*************新增结束**************/
...
GlobeSurfaceShaderSet.prototype.getShaderProgram = function (options) {
  /*************新增开始**************/
  if(options.isUpdatedSharder){
    let originVertexShader = this.baseVertexShaderSource.sources[2].toString();
    let originFragmentShader = this.baseFragmentShaderSource.sources[2].toString();
    ...
    let hasLight = false;
    for ( let key in options.lightMap ) {
      hasLight = true;
    }
    if(hasLight){
      let shader = getShader(options.lightMap,
          Cesium.ShaderSource.replaceMain(sourceVS,'sourceVS'),
          Cesium.ShaderSource.replaceMain(sourceFS,'sourceFS'));
      this.baseVertexShaderSource.sources[2] = shader.vs;
      this.baseFragmentShaderSource.sources[2] = shader.fs;
    }else{
      this.baseVertexShaderSource.sources[2] = sourceVS;
      this.baseFragmentShaderSource.sources[2] = sourceFS;
    }
  ...
  let tailor = options.applyTailor?true:false;
