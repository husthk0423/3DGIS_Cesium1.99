...
import HouseDataSource from './HouseDataSource';
...
const defer =Cesium.defer;
     ...
     */
    class HouseTileServiceImageryProvider extends ElevationImageryProvider{
        constructor(viewer,options) {
            ...
            this.ridingLanternHeight = defaultValue(
                options.ridingLanternHeight,
                200
            );

            this.ridingLanternColor = defaultValue(
                options.ridingLanternColor,
                '#4D80CD'
            );

            this.ridingLanternAlpha = defaultValue(
                options.ridingLanternAlpha,
                1.0
            );

            this.ridingLanternType = defaultValue(
                options.ridingLanternType,
                1
            );
            this.ridingLanternSpeed = defaultValue(
        ...
        requestJsonData(deferred,imageryProvider, url,xyz,request){
            ...
            let promise =  this.processor.scheduleTask({url:url.url,xyz:xyzStr,needDecode:this.options.needDecode,level:styleLevel,
        ...
        jsonPromiseResult(deferred,xyz,level,results){
            ...
            let name = x+'_'+y+'_'+z;
            let tile = new HouseDataSource(name,results,level,this.viewer,this.options,this.lightMap);
...
export default HouseTileServiceImageryProvider;
