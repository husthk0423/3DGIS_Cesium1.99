...
function canTransferArrayBuffer() {
  if (!defined(TaskCallBackProcessor._canTransferArrayBuffer)) {
    ...
    try {
      ...
      TaskCallBackProcessor._canTransferArrayBuffer = false;
      return TaskCallBackProcessor._canTransferArrayBuffer;
    }
    ...
    worker.onmessage = function (event) {
      ...
      TaskCallBackProcessor._canTransferArrayBuffer = result;
    };

    TaskCallBackProcessor._canTransferArrayBuffer = deferred.promise;
  }
...
function completeTask(processor, data) {
  ...
  const callbacks = processor._callbacks;
 ...
 */
function TaskCallBackProcessor(workerPath, maximumActiveTasks) {
  const uri = new Uri(workerPath);
  this._workerPath =
    uri.scheme().length !== 0 && uri.fragment().length === 0
      ? workerPath
      : TaskCallBackProcessor._workerModulePrefix + workerPath;
 ...
 *
 * @example
 * const TaskCallBackProcessor = new Cesium.TaskCallBackProcessor('myWorkerPath');
 * const promise = TaskCallBackProcessor.scheduleTask({
 *     someParameter : true,
 *     another : 'hello'
 * });
 ...
 * }
 */
TaskCallBackProcessor.prototype.scheduleTask = function (
  parameters,
  callbackNum,
  callbackFun,
  transferableObjects
  ...
  return Promise.resolve(canTransferArrayBuffer()).then(function (
    ...
    let id = 0;
    for(let i = 0;i<callbackNum;i++){
      id = processor._nextID++;
      processor._callbacks[id] = callbackFun;
      // const deferred = defer();
    }
 ...
 */
TaskCallBackProcessor.prototype.initWebAssemblyModule = function (webAssemblyOptions) {
  if (!defined(this._worker)) {
    this._worker = createWorker(this);
  }
 ...
 *
 * @see TaskCallBackProcessor#destroy
 */
TaskCallBackProcessor.prototype.isDestroyed = function () {
  return false;
};
 ...
 * <code>isDestroyed</code> will result in a {@link DeveloperError} exception.
 */
TaskCallBackProcessor.prototype.destroy = function () {
  if (defined(this._worker)) {
 ...
 * @private
 */
TaskCallBackProcessor.taskCompletedEvent = taskCompletedEvent;

// exposed for testing purposes
TaskCallBackProcessor._defaultWorkerModulePrefix = "Workers/";
TaskCallBackProcessor._workerModulePrefix = TaskCallBackProcessor._defaultWorkerModulePrefix;
TaskCallBackProcessor._canTransferArrayBuffer = undefined;
export default TaskCallBackProcessor;
