...
function ImageryLayer(imageryProvider, options) {
  ...
  this.emptyImage = document.createElement('canvas');
...
ImageryLayer.prototype.destroy = function () {
  /*************新增开始**************/
  if(this._imageryProvider.destroy){
...
ImageryLayer.prototype._requestImagery = function (imagery) {
  ...
  function success(image) {
    ...
    if(image.isTexture){
      let useWebMercatorT = imageryProvider.tilingScheme.projection instanceof WebMercatorProjection;
      if(useWebMercatorT){
        imagery.textureWebMercator = image.texture;
      }else{
        imagery.texture = image.texture;
      }
  ...
  function doRequest() {
    // 当第0级的瓦片下载很慢时，其它后续的瓦片不渲染，加上此行，后续的渲染不阻塞。
    if(imagery.level == 0){
      success(that.emptyImage);
    }
...
ImageryLayer.prototype.queueReprojectionCommands = function (frameState) {
  ...
  this._imageryProvider.frameState = frameState;
};
...
ImageryLayer.prototype.removeImageryFromCache = function (imagery) {
  ...
  /*************新增开始**************/
  if(this._imageryProvider.removeImageryFromCache){
    this._imageryProvider.removeImageryFromCache(imagery.x,imagery.y,imagery.level);
  }
  /*************新增结束**************/
};
