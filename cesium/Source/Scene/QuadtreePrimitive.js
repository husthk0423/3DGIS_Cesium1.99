...
QuadtreePrimitive.prototype.render = function (frameState) {
  ...
  if (passes.render) {
    ...
    /*************新增开始**************/
    let _imageryLayers = tileProvider._imageryLayers;
    for(let i = 0;i<_imageryLayers.length;i++){
      let _imageryProvider = _imageryLayers._layers[i]._imageryProvider;
...
function visitTile(
  ...
  let screenSpaceErrorValue = screenSpaceError(primitive, frameState, tile);
  let meetsSse = screenSpaceErrorValue < primitive.maximumScreenSpaceError;
  tile.meetsSse = meetsSse;
  meetsSse = needLoad(tile,meetsSse,screenSpaceErrorValue,primitive.maximumScreenSpaceError);
...
function visitIfVisible(
  ...
  } else if (primitive.preloadSiblings || (tile.level === 0 && primitive._tileProvider._terrainProvider.requestZeroLevel)) {
...
function needLoad(tile,meetsSse,screenSpaceErrorValue,maximumScreenSpaceError){
  if(!Cesium.showLevelMap){
    return meetsSse;
  }
  let index = Cesium.showLevelMap[tile.level];
  //如果当前瓦片为显示层级的最后一个层级，则直接显示
  if(index == Cesium.showLevelLength-1){
    return true;
  }
  ...
  if(index){
    if(meetsSse){
      ...
      //如果下一级可见
      if(screenSpaceErrorValue*0.5< maximumScreenSpaceError){
        let nextLevelIndex = Cesium.showLevelMap[tile.level+1];
        //如果下一级图层配置可以显示，本级图层则设置为false,否则设置为true
        if(nextLevelIndex){
          return false;
        }else{
          return true;
