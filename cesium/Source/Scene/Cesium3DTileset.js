...
function Cesium3DTileset(options) {
  ...
  this.tName = '3dtiles_' + md5(options.url);
  ...
  /**hk新增**/
  this.multiClippingPlanes=options.multiClippingPlanes;
  ...
  this.useIndexDB = defaultValue(
      options.useIndexDB,
      true
  );

  this.workerNum = defaultValue(
      options.workerNum,
      1
  );
  ...
  for(let i = 0; i< this.workerNum; i++){
    let processor = new Cesium.TaskProcessor('Cesium3DtilesWorker', Number.POSITIVE_INFINITY);
  ...
  this._readyPromise = Promise.resolve(options.url)
      .then(function(url){
        let option = {init:true,tName:that.tName,useIndexDB:that.useIndexDB};

        let defer1 = defer();

        let promises = [];
        for(let i = 0; i< that.workerNum; i++){
           let processor = that.processors[i];
           promises.push(processor.scheduleTask(option));
        }

        Promise.all(promises).then(function(result) {
          Cesium.processorMap[url] =that.processors;
          defer1.resolve(url);
        });
  ...
  while (stack.length > 0) {
    ...
    if (defined(children)) {
      ...
      for (let i = 0; i < length; ++i) {
        ...
        replaceSubdomain(resource,this._subdomains);
...
function replaceSubdomain(resource,subdomains){
  ...
  let index = Math.round(Math.random()*(subdomains.length - 1));
  let ToReplace = subdomains[index]+'/';
  // var reg = /(?<=^http(s)?:\/\/)(.*?)\//;
  // resource._url = resource._url.replace(reg, ToReplace);
  let url = resource._url;
  ...
  let relUrl = otherUrl.substring(start+1);
  resource._url =  protocolHeader + '//'+ ToReplace + relUrl;
}
...
Cesium3DTileset.prototype.prePassesUpdate = function (frameState) {
  ...
  if (defined(this.multiClippingPlanes)) {
    this.multiClippingPlanes.update(frameState);
  }
