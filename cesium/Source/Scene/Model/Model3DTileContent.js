...
Model3DTileContent.prototype.update = function (tileset, frameState) {
  ...
  if (defined(tileset.multiClippingPlanes) &&  model._ready &&
      (tileset.multiClippingPlanes._multiCollections.length != model._multiCollectionsLength ||
          tileset.multiClippingPlanes._clipOutSide != model._clipOutSide)) {
    model.multiClippingPlanes = tileset.multiClippingPlanes;
    model._multiCollectionsLength = tileset.multiClippingPlanes._multiCollections.length;
    model._clipOutSide = tileset.multiClippingPlanes._clipOutSide;
  }
...
function makeModelOptions(tileset, tile, content, additionalOptions) {
  const mainOptions = {
    ...
    testboundingVolume:tile.boundingVolume,

    multiClippingPlanes: tileset.multiClippingPlanes,
