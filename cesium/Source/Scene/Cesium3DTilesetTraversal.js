...
function selectDesiredTile(tileset, tile, frameState) {
  ...
  if (defined(loadedTile)) {
    //当前应该绘制的瓦片不为merge，并且父级可绘制的瓦片为merge的瓦片，则不用父级的merge瓦片
    if(tileset.immediatelyLoadDesiredLevelOfDetail &&  tileset.skipLevelOfDetail &&
        tile._contentResource._url.indexOf('merge') == -1 && loadedTile._contentResource._url.indexOf('merge') > -1
        && tile._depth - loadedTile._depth > 2){
      selectDescendants(tileset, tile, frameState);
      return;
    }

    // if(tileset.immediatelyLoadDesiredLevelOfDetail &&  tileset.skipLevelOfDetail &&
    //     tile._depth - loadedTile._depth >0){
    //   console.log('souceTlie:'+tile._contentResource._url);
    //   console.log('loadedTile:'+loadedTile._contentResource._url);
    //   selectDescendants(tileset, tile, frameState);
    //   return;
    // }

    // if(loadedTile._contentResource._url.indexOf('merge') > -1){
    //   console.log(tile._contentResource._url);
    // }
...
function executeTraversal(
  ...
  while (stack.length > 0) {
    ...
    if(tileset.immediatelyLoadDesiredLevelOfDetail &&  tileset.skipLevelOfDetail && stoppedRefining
        && tile.children.length > 0 && tile._contentResource && tile._contentResource._url.indexOf('.json') == -1
        && tile._screenSpaceError > tileset._maximumScreenSpaceError){
