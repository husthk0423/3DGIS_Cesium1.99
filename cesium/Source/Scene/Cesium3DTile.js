...
function requestSingleContent(tile) {
  ...
  let processors = tile._tileset.processors;
  let index = Math.round(Math.random()*(processors.length - 1));
  let processor = processors[index];

  let promise =  processor.scheduleTask({type:'3dtile',url:resource._url,throttleByServer:true});
  ...
  const contentReadyToProcessPromise = promise.then(function (arrayBuffer) {
    ...
    // console.log('11111111111:  '+new Date().getTime());
  ...
  tile._contentReadyPromise = contentReadyToProcessPromise
    .then(function (content) {
      ...
      // console.log('2222222222222:  '+new Date().getTime());
      --tileset.statistics.numberOfPendingRequests;
    ...
    .then(function (content) {
      ...
      // console.log('3333333333333:  '+new Date().getTime());
      // console.log(content._resource._url);
      if (tile.isDestroyed()) {
