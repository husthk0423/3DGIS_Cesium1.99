...
function WebMapTileServiceImageryProvider(viewer,options) {
    ...
    this.isDrawNow = defaultValue(options.isDrawNow, false);
    ...
    this.processor =   new Cesium.TaskProcessor('WmtsWorker', Number.POSITIVE_INFINITY);
...
function requestImage(imageryProvider, col, row, level, request, interval) {
    ...
    var deferred = defer();
    requestImageData(deferred,imageryProvider, resource,request);
    return deferred.promise;
    // return ImageryProvider.loadImage(imageryProvider, resource);
}

function requestImageData(deferred,imageryProvider, resource,request){
    let promise =  imageryProvider.processor.scheduleTask({url:resource.url});
    promise.then(function(results){
        ...
        let len = results.pixelFormat == Cesium.PixelFormat.RGBA ? 4 : 3;
        let tilesize = Math.sqrt(results.data.byteLength / len);
...
WebMapTileServiceImageryProvider.prototype.setFilter = function(filter) {
    ...
    WebMapTileServiceImageryProvider.prototype.UpdateUrl = function(url) {
        ...
        var resource = Resource.createIfNeeded(this.options.url);
