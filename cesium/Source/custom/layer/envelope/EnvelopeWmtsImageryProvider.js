...
import EnvelopeWmtsDataSource from './EnvelopeWmtsDataSource';
 ...
 */
class EnvelopeWmtsImageryProvider {
    constructor(viewer,options) {
        ...
        //与父级瓦片相差大于多少级时不显示
        this.offsetParentLevel = defaultValue(options.offsetParentLevel, 0);
        ...
        if(this._tileWidth == 512){
            this.offsetParentLevel = this.offsetParentLevel + 1;
        }
        ...
        this.bufferDistance = defaultValue(this.options.bufferDistance,0);
    }
        ...
        for(let i = 0;i<renderTiles.length;i++){
            ...
            for(let j = 0;j<tileImagerys.length;j++){
                ...
                if(imagery && imagery.imageryLayer._imageryProvider.id == this.id){
                    if(tile.level - imagery.level > this.offsetParentLevel){
                        continue;
                    }
    ...
    requestJsonData(deferred,imageryProvider, url,xyz,request){
        ...
        let promise =  this.processor.scheduleTask({url:url.url,xyz:xyzStr,level:level,
        ...
        promise.then(function(deferred,xyz,level,request,results){
            ...
            // if(results instanceof ArrayBuffer){
                results = new Cesium.Texture({
                    context: this.viewer.scene.frameState.context,
                    pixelFormat: results.pixelFormat,
                    width: this.tileWidth,
                    height: this.tileWidth,
                    source: {
                        arrayBufferView: new Uint8Array(results.data),
                    },
                });
            // }
            this.jsonPromiseResult(deferred,xyz,level,results,rectangle);
    ...
    jsonPromiseResult(deferred,xyz,level,results,rectangle){
        ...
        let name = x+'_'+y+'_'+z;
        let tile = new EnvelopeWmtsDataSource(name,results,rectangle,this.tileWidth,level,this.viewer,this.options,this.lightMap);
...
export default EnvelopeWmtsImageryProvider;
