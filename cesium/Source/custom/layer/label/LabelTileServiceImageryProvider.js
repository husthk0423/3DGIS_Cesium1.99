...
import GlyphSource from './glyph/GlyphSource';
import AvoidTile from './AvoidTile';
import Buffer from '../../utils/Buffer';
import VarintReader from '../../utils/VarintReader';

import ElevationImageryProvider from '../ElevationImageryProvider';
import ElevationTool from '../../utils/ElevationTool';
import Control from '../vector/parse/Control';
const defer =Cesium.defer;
    ...
    class LabelTileServiceImageryProvider extends ElevationImageryProvider{
        constructor(viewer,options) {
            ...
            //注记整体偏移的高度
            this.offsetHeight = defaultValue(options.offsetHeight, 0);
            ...
            let promise3 = ElevationTool.getDBMap(this.indexDbNames,this.dbMap);
            promises.push(promise3);

            Promise.all(promises).then(function(data){
                let results = data[2];
                if (results && results.layerMap) {
                    this.propertyConfigMap = results.propertyConfigMap;
                    if(this.controlStr && !this.control){
                        this.control = new Control(this.propertyConfigMap);
                        this.control = this.control.strToFilterControl(this.controlStr);
                    }
         ...
         */
        setFilter(filter) {
            this.controlStr = JSON.stringify(filter);
            if(this.dataType == 'binary'){
                if(this.propertyConfigMap){
                    this.control = new Control(this.propertyConfigMap);
                    this.control = this.control.strToFilterControl(this.controlStr);
                }
            }else{
        ...
        requestJsonData(deferred,imageryProvider, url,xyz){
            ...
            let p1 = Promise.all(promises).then(function(result){
                ...
                // const promise1 = this.viewer.scene.sampleHeightMostDetailed(labelTile.cartographics);

                const xyzStr = xyz.x+'_'+xyz.y+'_'+xyz.z;
                const promise1 = ElevationTool.getElevation(this.dbMap,this.indexDbNames,xyzStr);
           ...
           let p2 = p1.then(function(result){
               ...
               deferred.resolve(this.image);
                // labelTile.updateLabelPostion(heights);
                // return new Promise(function(resolve, reject){
                //     resolve(null);
                // });
            }.bind(this));

            // p2.then(function(elevationDataMap){
            //     if(elevationDataMap){
            //         labelTile.updateElevationData(elevationDataMap);
            //     }
            //     labelTile.ready = true;
        ...
        jsonPromiseResult(xyz,level,data){
            ...
            if(data !={}){
                ...
                let labelTile = this.cache[name];
                if(!labelTile){
                    labelTile = new LabelTile(this.offsetHeight,this.defaultHeight,name,xyz,this.tileWidth,rectangle,labelData,this.indexDbNames,this.control,this.styleFun,this.dataType,
...
export default LabelTileServiceImageryProvider;
