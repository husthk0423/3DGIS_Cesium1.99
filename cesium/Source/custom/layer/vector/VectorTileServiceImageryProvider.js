...
import ProgramFactory from '../shaders/ProgramFactory';
import TextureQueue from '../../utils/TextureQueue';
import LineAtlas from './LineAtlas';
import DrawFboVector from './draw/DrawFboVector';
import DrawTilesFboVector from './draw/DrawTilesFboVector';
...
const defer =Cesium.defer;
    ...
    class VectorTileServiceImageryProvider {
        constructor(viewer,options,shaders) {
            ...
            if(options.hasOwnProperty('serverName') && options.hasOwnProperty('styleId')
        ...
        updateUrl(url) {
            ...
            let resource = Resource.createIfNeeded(this.options.url);
            ...
            if(this.options.hasOwnProperty('serverName') && this.options.hasOwnProperty('styleId')
                && this.options.hasOwnProperty('host')){
        ...
        draw(tilesToRender){
            ...
            for(let name in renderTileMap){
                ...
                if(tile){
                    ...
                    if(tile.styleId != this.styleId){
                        let promise = this.changeStyle(tile,styleLevel,key);
            ...
            if(this.changStylePromise.length  > 0){
                Promise.all(this.changStylePromise).then(function(){
                    ...
                    // this.textureQueue.empty();
                    this.changStylePromise = [];
                    DrawTilesFboVector(this,this.needDrawTiles,this.needDrawTilesMap,null,true);
                }.bind(this));
                return;
            }
            DrawTilesFboVector(this,this.needDrawTiles,this.needDrawTilesMap,null);
        }
...
export default VectorTileServiceImageryProvider;
