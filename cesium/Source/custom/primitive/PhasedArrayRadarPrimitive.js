// 相控阵雷达范围
import PhasedArrayRadar from '../shaders/PhasedArrayRadar';
import PhasedArrayRadarFS from '../shaders/PhasedArrayRadarFS';
import PhasedArrayRadarVS from '../shaders/PhasedArrayRadarVS';
import PhasedArrayRadarScanPlaneFS from '../shaders/PhasedArrayRadarScanPlaneFS';
...
let Buffer = Cesium.Buffer;
let BufferUsage = Cesium.BufferUsage;
let DrawCommand = Cesium.DrawCommand;
let Pass = Cesium.Pass;
let RenderState = Cesium.RenderState;
let ShaderProgram = Cesium.ShaderProgram;
let ShaderSource = Cesium.ShaderSource;
let VertexArray = Cesium.VertexArray;
let BlendingState = Cesium.BlendingState;
let CullFace = Cesium.CullFace;
...
function computeUnitPosiiton(primitive, xHalfAngle, yHalfAngle) {
  ...
  //以中心为角度
  let cosYHalfAngle = cos(yHalfAngle);
  let tanYHalfAngle = tan(yHalfAngle);
  let cosXHalfAngle = cos(xHalfAngle);
  let tanXHalfAngle = tan(xHalfAngle);
...
function computeSectorPositions(primitive, unitPosition) {
  let xHalfAngle = primitive.xHalfAngle,
...
function createCommonShaderProgram(primitive, frameState, material) {
  ...
  let vs = PhasedArrayRadarVS;
  let fs = new ShaderSource({
    sources: [PhasedArrayRadar, material.shaderSource, PhasedArrayRadarFS],
  });
...
function createScanPlaneShaderProgram(primitive, frameState, material) {
  ...
  let vs = PhasedArrayRadarVS;
  let fs = new ShaderSource({
    sources: [PhasedArrayRadar, material.shaderSource, PhasedArrayRadarScanPlaneFS],
  });
...
export default  PhasedArrayRadarPrimitive;
