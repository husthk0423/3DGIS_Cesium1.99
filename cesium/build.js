...
export async function buildCustomJs(options) {
  // const css = await globby(cssFiles);
  const buildConfig = esbuildBaseConfig();
  buildConfig.entryPoints = ["Source/Custom.js"];
  ...
  if (options.iife) {
    const result = await esbuild.build({
      ...
      format: "iife",
      globalName: "Custom",
      outfile: path.join(options.path, "CustomCesiumSDK.js"),
    });
...
const customSourceFiles = [
  "Source/custom/**",
  "!Source/custom/layer/shaders/glsl/*.glsl",
];
...
export async function createCustomJs() {
  let contents = `export const VERSION = '${version}';\n`;
  const files = await globby(customSourceFiles);
  files.forEach(function (file) {
    file = path.relative("Source", file);
  ...
  await writeFile("Source/Custom.js", contents, { encoding: "utf-8" });
...
// export function mergeCustom(options) {
//   options = options || {};
//
//   return esbuild.build({
//     entryPoints: [
//       "Build/CesiumUnminified/Cesium.js",
//       // "Build/CustomCesiumSDK.js",
//     ],
//     // bundle: false,
//     format: "iife",
...
//     plugins:  undefined,
//     outfile: path.join(options.path, "Cesium1.js"),
//     // outdir: path.join('Build', "Cesium1"),
//     incremental: false,
//     write: true,
//   });
// }
